...
public class ToscaParser {
  ...
  /**
   * Creates an object to represent the sustainability policy.
   *
   * @param policyName Name of the policy
   * @param entry Policy data.
   * @return The policy object
   */
  private SustainabilityPolicy createSustainabilityPolicy(String policyName, Map<String, Object> entry) {
    Map<String, Object> props = (Map<String, Object>) entry.get("properties");
    Object footprint = props != null ? props.get("max_total_carbon_footprint") : null;

    // Se for um valor numérico direto (expresso em tCO2e)
    ...
    if (footprint instanceof Map) {
      System.out.println("Atenção: Valor de carbono é dinâmico (get_input); usando limite ilimitado (sem restrição).");
    }

    // Sem limite explícito -> sentinela de "sem limite"
    return new SustainabilityPolicy(policyName, SustainabilityPolicy.UNLIMITED);
  }
